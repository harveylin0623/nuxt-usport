<template>
  <div class="flex flex-row items-center justify-center gap-[2px] pt-5">
    ...
    <button
      ...
      class="size-9 rounded-full bg-icon-arrow-right bg-center bg-no-repeat disabled:opacity-25"
      @click="nextPage"
    ></button>
  </div>
</template>
...
const buttonStyle = computed(() => item => item === currentPage.value ? 'bg-primary-1 text-white' : 'bg-transparent text-neutral-4')
